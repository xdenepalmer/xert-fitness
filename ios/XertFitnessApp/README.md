...
- Live member notices authored in the admin command centre, with priority and automatic expiry shared across web and iOS.
...
Before starting a signed build, run `src/supabase/release_readiness_check.sql` in the production Supabase SQL editor. All twelve rows must show `installed = true` and `release_ready = true`; otherwise the service-contract preflight stops before signing and names the missing capability.
