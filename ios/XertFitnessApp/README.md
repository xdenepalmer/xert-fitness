...
Before starting a signed build, run `src/supabase/release_readiness_check.sql` in the production Supabase SQL editor. Every row must show `installed = true` and `release_ready = true`; otherwise the service-contract preflight stops before signing and names the missing capability. The member booking-switch guard makes **Member App Controls → Bookings enabled** authoritative for website and iOS clients at the database boundary.
