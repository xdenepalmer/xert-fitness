...
import { ADMIN_BUTTON, ADMIN_PAGE, AdminPageHeader, AdminFilterBar, AdminDataTable, AdminFormField, AdminBadge } from '@/components/admin/ui';
import './leads.css';
...
export default function LeadTable({ type = 'member', onDirtyChange }) {
  ...
  const columns = [
    { key: 'full_name', header: 'Lead', render: lead => <div className="lead-contact"><button type="button" className="lead-open" disabled={bulkSaving} onClick={() => setSelectedLead(lead)}>{leadLabel(lead)}</button><span className="lead-secondary">{lead.email}</span>{lead.phone && <span className="lead-secondary">{lead.phone}</span>}</div> },
    { key: 'status', header: 'Status', render: lead => <AdminBadge status={lead.status}>{(lead.status || 'Unknown').replace(/_/g, ' ')}</AdminBadge> },
    { key: 'context', header: type === 'member' ? 'Training goals' : type === 'trainer' ? 'Qualifications' : 'Business', render: lead => type === 'member' ? lead.main_training_goals?.slice(0, 3).join(', ') || '—' : type === 'trainer' ? lead.qualifications || '—' : lead.business_name || lead.profession || '—' },
    { key: 'created_at', header: 'Submitted', render: lead => <div className="lead-contact"><time dateTime={lead.created_at}>{new Date(lead.created_at).toLocaleDateString('en-AU')}</time>{lead.utm_source && <span className="lead-secondary">{lead.utm_source}</span>}</div> },
  ];
  ...
  return <div className={`${ADMIN_PAGE} lead-pipeline`} data-lead-pipeline={type}>
    ...
    {selectedLead && <LeadDetailDrawer key={selectedLead.id} lead={selectedLead} statuses={statuses} table={table} onClose={() => setSelectedLead(null)} onUpdate={() => load(1)} onDirtyChange={onDirtyChange} />}
  </div>;
}
