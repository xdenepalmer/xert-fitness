...
export function AdminDrawer({open, onOpenChange, title, description = null, children, footer = null, trigger = null, role = 'dialog', closeLabel = 'Close drawer', closeDisabled = false}) {
  ...
  return <>{triggerElement}{open && typeof document !== 'undefined' && createPortal(
    <dialog ref={dialog} role={role} className="admin-drawer" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined}
      ...
      <div className="admin-drawer-close-region"><button type="button" className="admin-kit-button" disabled={closeDisabled} aria-label={closeLabel} onClick={() => onOpenChange(false)}>Close</button></div>
