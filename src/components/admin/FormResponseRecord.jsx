...
function SignatureAnswer({ value }) {
  ...
  return <div className="xert-response-signature border-b border-slate-400 pb-2"><img alt="Respondent signature" className="max-h-36 max-w-full object-contain object-left" src={source} /></div>;
}
    ...
    <div className="space-y-4">
      ...
      <article ref={recordRef} className="xert-response-print-record mx-auto max-w-[850px] bg-white text-slate-950 shadow-2xl shadow-black/30" aria-labelledby="response-record-title">
        ...
        <div className="px-5 py-6 sm:px-10 sm:py-8">
          ...
          {definition.isReconstructed && <aside className="mt-4 border border-amber-700/40 bg-amber-50 p-3 text-xs leading-5 text-amber-950"><strong>Legacy reconstructed record.</strong> The submitted answer values are original, but this record was reconstructed from the current form. Exact original question labels and layout cannot be guaranteed.</aside>}
          ...
          <footer className="mt-10 border-t border-slate-300 pt-4 text-[10px] leading-4 text-slate-500"><p>{definition.usesSubmissionSnapshot ? 'Rendered from the form definition preserved at submission.' : 'Legacy record reconstructed from the current form; exact original labels and layout cannot be guaranteed.'}</p><p className="mt-1">Private administrative record · Generated by XERT Fitness</p></footer>
        </div>
      </article>
    </div>
  );
