...
-- No Zapier credential is stored. The optional Get User mirror retains only
-- bounded admin-only name/email/phone fields plus provider status/freshness.
...
create table if not exists public.fitbox_integration_jobs (
  ...
  constraint fitbox_integration_jobs_type_check check (job_type in ('register_prospect', 'get_user')),
...
create table if not exists public.fitbox_member_links (
  ...
  constraint fitbox_member_links_profile_first_name_check check (profile_first_name is null or char_length(profile_first_name) <= 80),
  constraint fitbox_member_links_profile_last_name_check check (profile_last_name is null or char_length(profile_last_name) <= 80),
  constraint fitbox_member_links_profile_email_check check (profile_email is null or char_length(profile_email) <= 320),
  constraint fitbox_member_links_profile_phone_check check (profile_phone is null or char_length(profile_phone) <= 60)
);
...
-- Completes the verified read-only Get User search. Only the FitBox mirror row
-- is refreshed; XERT leads, auth profiles, bookings, Stripe and access remain
-- untouched. DOB is deliberately not accepted because discovery returned an
-- unverified provider default for a prospect created without one.
...
revoke execute on function public.complete_fitbox_get_user_job(uuid, text, text, text, text, text, text, text, text) from public, anon, authenticated;
...
grant execute on function public.complete_fitbox_get_user_job(uuid, text, text, text, text, text, text, text, text) to service_role;
