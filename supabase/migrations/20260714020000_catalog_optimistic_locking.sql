...
begin
  ...
  from jsonb_to_record(p_product) as product_data(
    ...
    sessions_count integer, validity_days integer, featured boolean, sort_order integer
  );

  if p_slug is null or length(p_slug) > 80 or p_slug !~ '^[a-z0-9]+(-[a-z0-9]+)*$'
     or v_product.name is null or btrim(v_product.name) = '' or length(btrim(v_product.name)) > 120
  ...
  insert into public.products (
    ...
    p_slug, btrim(v_product.name), nullif(btrim(v_product.description), ''),
    v_product.price_cents, lower(v_product.currency), v_product.sessions_count,
    v_product.validity_days, null, v_product.featured, false, v_product.sort_order
