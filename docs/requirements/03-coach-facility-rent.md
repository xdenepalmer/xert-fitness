...
- **New `api/coach-rent-invoice.js`** — admin-only, following `api/admin-refund-order.js` line for line: `createRequestTrace`, bearer token → `admin.auth.getUser`, `profiles.role !== 'admin'` → 403, `inspectCommerceRuntimeEnvironment(process.env)` gate, `createXertStripeClient`. Actions:
  - `create`: reads the issued statement, upserts the Stripe Customer from `coach_engagements.stripe_customer_id` (creating with `metadata.xert_coach_id`), creates an `invoiceItem` per breakdown line (base licence fee; tier component; GST as a Stripe `tax_rate` of 10% GST rather than a hand-rolled line, so the PDF is a compliant tax invoice), then `stripe.invoices.create({ customer, collection_method: 'send_invoice', days_until_due: 7, currency: 'aud', auto_advance: true, metadata: { xert_statement_id, xert_coach_id, xert_statement_month } })`, finalises and sends. Idempotency key `xert-coach-rent:${statement.id}` mirrors `checkoutIdempotencyKey`.
  - `void`: `stripe.invoices.voidInvoice` then `admin_void_coach_statement`.
  Every Stripe object carries `metadata.xert_statement_id`, and the webhook refuses any invoice lacking it — the same identity discipline as `hasXertCheckoutIdentity`.

- **Extend `api/stripe-webhook.js`** — add `invoice.finalized`, `invoice.paid`, `invoice.payment_failed`, `invoice.marked_uncollectible` to the handled set. Add `coachRentSettlementForEvent(event)` beside `checkoutFulfillmentForEvent`, gated on `metadata.xert_statement_id` matching the UUID pattern, and route it through the existing `processStripeEvent` ledger so retries, duplicates and `finish_stripe_webhook_event` behave identically. **Do not add a second webhook endpoint** — the ledger in `stripe_webhook_events` is the thing that makes delivery auditable.
...
- `src/components/coach/CoachRoute.jsx` — the mirror of `AdminRoute.jsx`, but gating on a coach engagement rather than `profiles.role`. Extend `src/lib/SupabaseAuthContext.jsx` to expose `coachId` and `coachLoading` by calling `supabase.rpc('current_coach_id')` alongside the existing profile fetch (guarded so a null result is the normal case for members and costs one cheap RPC). Loader → sign-in → "no coach engagement" screen with a link to `/account`.
- `src/pages/CoachPortal.jsx` — the coach's whole world, wrapped in `PublicNav` / `PublicFooter` like `Account.jsx`. Sections: **This month** (rent owed, due date, pay-now link to the Stripe hosted invoice, active-client count so far); **Book the floor** (`CoachSessionLogger`); **My clients**; **Statements**; **My rent** (`CoachRentExplainer`); **My agreement**.
...
**Authorisation model.** Do **not** add `'coach'` to `profiles.role`. `admin_set_role` hard-codes `if p_role not in ('member','admin')`, `admin_role_changes` has `check (… in ('member','admin'))` on both role columns, `memberAdmin.js:30` and `:53` whitelist the same two values, and every RLS policy in the database is shaped `is_admin() or own-row`. A third role means re-auditing all of it for a benefit that a link table gives for free. Instead, `coach_engagements.user_id` links a coach to an auth user and `public.current_coach_id()` / `public.current_active_coach_id()` are the scope predicates. Coaches keep `role = 'member'` — which is genuinely useful, since a coach who trains at XERT can also buy packs and book classes with no special-casing.
...
**Money handling.** Stripe secrets never reach the browser: invoice creation is server-side in `api/coach-rent-invoice.js` behind bearer-token + `profiles.role = 'admin'`, exactly as `api/admin-refund-order.js` does. Webhook events must carry `metadata.xert_statement_id` matching the UUID pattern or they are ignored, mirroring `hasXertCheckoutIdentity`. Events flow through the existing `stripe_webhook_events` ledger so retries and duplicates behave identically to session-pack fulfilment. `settle_coach_statement_invoice` is `service_role` only.
