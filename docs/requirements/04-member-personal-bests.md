...
Important architectural constraint you must design around: **this codebase has no coach role.** `profiles.role` is only `'member' | 'admin'` (`src/supabase/booking_schema.sql:16`, guarded by `guard_profile_write`, constrained again by `admin_role_changes` check `in ('member','admin')`, and protected by the last-admin guard in `admin_set_role`). `public.coaches` is a CMS table with no auth link and `class_sessions.coach_name` is free text. Do NOT add a third role for this feature — it touches `guard_profile_write`, `admin_set_role`, `admin_role_changes`, and every RLS policy in `rls_hardening.sql`. Instead: verification authority is `public.is_admin()`, and the verifying staff member must additionally attribute the verification to a row in the existing `coaches` table via `verified_coach_id`, which is what members actually want to see ("verified by Coach Jess"). That gives you coach attribution today with zero blast radius.
...
7. **Verification workload.** *Default: any admin may verify, and must attribute the verification to a row in the existing `coaches` table.* If the owner wants coaches who are not admins to verify, that requires a real coach role and roughly a week of work across `guard_profile_write`, `admin_set_role`, `admin_role_changes` and every RLS policy — quote it separately, do not smuggle it into this feature.
