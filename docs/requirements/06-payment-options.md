...
5. **Coach rent is the same subscription with a different payer.** `billing_plans.kind='coach_rent'`, `billing_subscriptions.coach_id` instead of `user_id`, `included_sessions is null`, no entitlement minting. Spec 03 owns engagement / sessions / reviews / statements and **consumes this spine** for collection — do not accept a parallel Stripe Invoicing integration. Mandate links may still be emailed, but **coach login is required** (per [spec 07](07-staff-accounts-and-roles.md) + 03’s portal) for sessions, safety roster, and rent visibility; the older “no XERT login” line is superseded.
...
- **Spec 03 is a consumer of this spine** for coach-rent collection ([INTEGRATION_REVIEW.md](INTEGRATION_REVIEW.md) §2C). One webhook endpoint and ledger; packs stay `mode=payment` + `fulfill_stripe_checkout`.
- **Coach login** for rent/ops is required by 07 + 03; mandate email alone is not enough (supersedes earlier “no XERT login” wording above).
