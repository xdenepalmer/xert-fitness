...
export default async function handler(request, response) {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) return json({ error: 'Member push delivery is not configured.' }, 500);

  try {
    const body = await requestJson(request);
    const sessionId = String(body?.session_id || '').trim();
    if (!UUID.test(sessionId)) return json({ error: 'A valid class session is required.' }, 400);
    ...
    if (!announcement) {
      return json({ error: 'No member-account cancellation notice was created for this class.' }, 404);
    }
    ...
    if ((previous || []).length > 0) {
      return json({ announcement_id: announcement.id, recipients: userIds.length, push: summarizePreviousClassAlertPushes(previous) });
    }
    ...
    return json({ announcement_id: announcement.id, recipients: userIds.length, push });
  } catch {
