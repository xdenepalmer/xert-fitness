...
export const commandIds = Object.freeze({
  ...
  fullClass: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc',
...
export function installCommandData(data, { mutations = [] } = {}) {
  ...
  const full = { ...future, id: commandIds.fullClass, session_id: commandIds.fullClass, title: 'Full Strength', status: 'full', capacity: 1 };
  data.class_sessions.push(full);
  ...
  const rosters = new Map([
    ...
    [full.id, [rosterRow(members[0], 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', 'confirmed')]],
  ]);
  data.xert_forms = [{
  ...
  return {
    read(name, args) {
      ...
      if (name === 'admin_class_capacity') return data.class_sessions.map(row => ({
        class_session_id: row.id, capacity: row.id === full.id ? 1 : 8, taken: rosters.get(row.id)?.filter(booking => booking.status === 'confirmed').length || 0,
        waiting: rosters.get(row.id)?.filter(booking => booking.status === 'waitlisted').length || 0, pending: 0, spots_left: row.id === full.id ? 0 : 6, booking_mode: 'request_to_book',
      }));
      return undefined;
    },
    mutate(name, { method, url, body }) {
      ...
      if (name === 'admin_book_member_into_class') {
        mutations.push({ name, body });
        if (![future.id, full.id].includes(body.p_session_id) || body.p_member_id !== commandIds.reese || !body.p_request_id) return failure('Unexpected fixture attendee request.');
        if (requestReceipts.has(body.p_request_id)) return { body: requestReceipts.get(body.p_request_id) };
        const roster = rosters.get(body.p_session_id);
        if (roster.some(row => row.member_id === body.p_member_id)) return failure('ALREADY_BOOKED');
        const bookingId = body.p_session_id === full.id ? 'ffffffff-ffff-4fff-8fff-ffffffffffff' : 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';
        const bookingStatus = body.p_session_id === full.id ? 'waitlisted' : 'confirmed';
        roster.push(rosterRow(members[1], bookingId, bookingStatus));
        const receipt = { request_id: body.p_request_id, session_id: body.p_session_id, member_id: commandIds.reese, booking_id: bookingId, booking_status: bookingStatus, credit_batch_id: null, announcement_id: null, created_at: '2026-01-01T00:00:00Z' };
