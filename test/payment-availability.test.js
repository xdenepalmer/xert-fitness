...
test('owner activation requires a fresh server preflight and an explicit confirmation', async () => {
  ...
  assert.match(webAdmin, /pendingPaymentActivationRef/);
  assert.match(webAdmin, /pendingPaymentActivationRef\.current = normalized/);
  assert.match(webAdmin, /if \(!pendingPaymentActivationRef\.current\)/);
  assert.match(webAdmin, /clearPendingPaymentActivation/);
  assert.match(webAdmin, /mutationsLocked/);
  assert.match(webAdmin, /disabled=\{mutationsLocked\}/);
