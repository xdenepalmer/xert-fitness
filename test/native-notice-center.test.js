...
test('push navigation carries the exact notice into Home', () => {
  ...
  assert.match(root, /consumePendingAnnouncementID\(\)[\s\S]*announcementID = pendingAnnouncementID[\s\S]*announcementNavigationRequest \+= 1[\s\S]*navigation\.select\(\.home, source: \.pushNotification\)/);
});
