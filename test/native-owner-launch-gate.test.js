...
test('native owner launch gate distinguishes paused preflight from live launch', async () => {
  ...
  assert.match(model, /static let totalChecks = 4/);
  ...
  // Payments and memberships are handled in Fitbox: Stripe checkout, linked
  // packs and the internal payment switch no longer gate the member launch.
...
test('native Operations Health requires real booking and member-notification evidence', async () => {
  ...
  assert.match(view, /private var bookingsSwitch: Bool\?/);
});
