...
test('operator upgrade stays re-run safe without matching the historical migration byte-for-byte', () => {
  ...
  assert.match(source, /install_guard_admin_schedule_change/);
  assert.match(source, /keeping newer guard_admin_schedule_change/);
  assert.match(migration, /create or replace function public\.guard_admin_schedule_change\(\)/i);
  assert.match(migration, /raise exception 'SCHEDULE_AUDIT_IMMUTABLE'/);
});
